    ...
    public void checkPaymentDeadlines() {
        ...
        try {
            ...
            for (BizReceiptPlan plan : plans) {
                ...
                // 通知创建者
                String title = String.format("【回款提醒】第%d期收款计划%s到期",
                        plan.getPlanNo(), days == 0 ? "今天" : "还剩" + days + "天");
                String content = String.format("计划金额: %s，计划日期: %s",
                        plan.getPlanAmount(), plan.getPlanDate());

                notificationService.notify(plan.getCreatorId(), title, content,
                        "payment_deadline", plan.getId());
